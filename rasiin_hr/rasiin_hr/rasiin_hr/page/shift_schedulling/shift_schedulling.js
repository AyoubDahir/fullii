...
// 	new ShiftSchedule(wrapper);
// };

// // ------------------------------------------------------------
// // CLASS
// // ------------------------------------------------------------
// ShiftSchedule = Class.extend({
...
// 	// -------------------------------------------------------
// 	// make_body() {
// 	// 	$(frappe.render_template("<div id='schedule'></div>")).appendTo(this.page.main);
// 	// },
...
// 		// Month filter
// 		let month_field = this.page.add_field({
// 			label: "Month",
// 			fieldtype: "Select",
// 			fieldname: "month",
// 			options: this.months(),
// 			default: this.curMonth,
// 			change() {
// 				me.curMonth = month_field.get_value();
// 				me.setup_datatable();
// 			}
// 		});
...
// 			options: "Designation",
// 			change() {
// 				me.designation = [desg.get_value(), desg2.get_value()];
// 				me.setup_datatable();
// 			}
// 		});
...
// 		// Employee
// 		let emp_filter = this.page.add_field({
// 			label: "Employee",
// 			fieldtype: "Link",
// 			options: "Employee",
// 			fieldname: "emp_filter",
// 			change() {
// 				me.emp_filter = emp_filter.get_value();
// 				me.setup_datatable();
// 			}
// 		});
// 	},
...
// 	// -------------------------------------------------------
// 	// MAIN FUNCTION TO BUILD DATATABLE
// 	// -------------------------------------------------------
// 	setup_datatable() {
// 		let me = this;
...
// 		let selMonth = months.indexOf(me.curMonth);

// 		let calendar = showCalendar(selMonth, currYear);
...
// 		let columns = [
// 			{ formatter: "rowSelection", titleFormatter: "rowSelection", frozen: true, headerSort: false },
// 			{ title: "ID", field: "employee", frozen: true, headerFilter: "input" },
// 			{ title: "Name", field: "employee_name", frozen: true, headerFilter: "input" }
// 		];

// 		calendar.forEach((r, index) => {
// 			let [dayNum, dayName] = r.split(" ");

// 			let month_num = String(selMonth + 1).padStart(2, "0");
// 			let day_num = String(index + 1).padStart(2, "0");
// 			let date_key = `${currYear}-${month_num}-${day_num}`;

// 			columns.push({
// 				title: dayNum,
// 				columns: [
// 					{ title: dayName, field: date_key, editor: "list", editorParams: { values: options } }
// 				]
// 			});
// 		});
...
// 		// EMPLOYEE QUERY
// 		// ---------------------
// 		let emp_filters = { status: "Active" };

// 		if (me.designation.length > 0)
// 			emp_filters["designation"] = ["in", me.designation];

// 		if (me.department)
// 			emp_filters["department"] = me.department;

// 		if (me.emp_filter)
// 			emp_filters["employee"] = me.emp_filter;
...
// 			// ---------------------
// 			// LOAD ALL SCHEDULES ONCE (FAST!)
// 			// ---------------------
// 			frappe.db.get_list("Employee Schedulling", {
// 				fields: ["employee", "employee_name", "shift", "label"],
// 				filters: {
// 					month: me.curMonth,
// 					year: currYear
// 				},
...
// 				all_sched.forEach(row => {
// 					if (!grouped[row.employee])
// 						grouped[row.employee] = {};

// 					// Convert shift to short code
// 					let s = row.shift;
// 					if (s === "Day Shift") s = "D";
// 					if (s === "Night Shift") s = "N";
// 					if (s === "Day and Night Shift") s = "D & N";
// 					if (s === "Free") s = "Off";
// 					if (s === "Canteen") s = "C";

// 					grouped[row.employee][row.label] = s;
// 					grouped[row.employee]["employee"] = row.employee;
// 					grouped[row.employee]["employee_name"] = row.employee_name;
// 				});
...
// 				// 	// Only include employees that have at least ONE shift
// 				// 	if (row && Object.keys(row).length > 2) {  
// 				// 		// > 2 because row contains {employee, employee_name} too
// 				// 		datatbl.push(row);
// 				// 	}
// 				// });
...
// 				// RENDER TABULATOR
// 				// ---------------------
// 				let table = new Tabulator("#schedule", {
// 					height: "400px",
// 					columns: columns,
// 					data: datatbl
// 				});
...
// 			let dayName = dayDefault[d.getDay()];

// 			result.push(`${date} ${dayName}`);
// 			days_no.push(date);
